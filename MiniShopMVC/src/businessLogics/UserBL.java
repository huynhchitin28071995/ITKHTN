...
public class UserBL {
	...
	public int add(User obj) {
		obj.setUserId(Helper.randomLong());
		int a = jdbc.update("INSERT INTO User(UserId, Username, Password, Email, Gender, Tel, Address) VALUES(?, ?, ?, ?, ?, ?, ?)", obj.getUserId(), obj.getUsername(), Helper.bcrypt(obj.getPassword()), obj.getEmail(), obj.getGender(), obj.getTel(), obj.getAddress());
		int b  = jdbc.update("INSERT INTO UserInRole(UserId, RoleId) VALUES(?, ?)", obj.getUserId(), 1);
		return a + b;
	}
	
	public static User logon(String username) {
		User user = jdbc.queryForObject("SELECT UserId, Username, Password FROM User WHERE Username = ?", new RowMapper<User>(){
			@Override
		...
		user.setRoles((Set<Role>) getRolesByMemberId(user.getUserId()));
		return user;
	}
